...
class UserController extends Controller
{
    ...
    public function users_update(Request $request)
    {
        $user_info = $request->validate([
            ...
            'middle_initial'   => 'required|max:100|alpha',
    ...
    public function invoice()
    {
            ...
            $users = DB::select('select year(created_at) as year, month(created_at) as month, sum(total) as total_amount from orders group by year(created_at), month(created_at)');

            $sum = DB::table('orders')
            ->sum('total');

            $orders = Order::with('inventory')->get();
            //$orders = Order::where('id',$id)->sum('total');


        //return view('admin.invoice',compact('users','sum',));
            $data = [
            'users' => $users,
            'sum' => $sum,
            'orders' => $orders,
